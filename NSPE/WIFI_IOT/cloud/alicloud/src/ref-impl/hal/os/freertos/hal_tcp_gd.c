...
uintptr_t HAL_TCP_Establish(const char *host, uint16_t port)
{
    ...
    sprintf(service, "%u", port);
     ...
     while(dns_retry++ < 8) {
        ...
        if (rc != 0) {
            platform_err("getaddrinfo error[%d], rc: %d, host: %s, port: %s\n", dns_retry, rc, host, service);
            sys_ms_sleep(1000);
            continue;
        }else{
            break;
    ...
    if (rc != 0) {
        platform_err("getaddrinfo error(%d), host = '%s', port = [%d]\n", rc, host, port);
        return (uintptr_t)(-1);
    }
    ...
    do {
        ...
        if (t_left >= 0) {
            ...
            if (ret > 0) {
                ...
                platform_err("select-write fail, ret = select() = %d", ret);
                net_err = 1;
                break;
            }
        }
        ...
        if (ret > 0) {
            ...
            if (ret > 0) {
                ...
                platform_err("send fail, ret = send() = %d", ret);
                net_err = 1;
                break;
            }
        }
